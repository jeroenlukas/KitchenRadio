...
bool VS1053::testComm(const char *header) {
    ...
    Serial.println(headerCr.c_str());  // Show a header
    ...
    for (i = 0; (i < 0xFFFF) && (cnt < 20); i += delta) {
        ...
        {
            Serial.printf("VS1053 error retry SB:%04X R1:%04X R2:%04X" , i, r1, r2);
            cnt++;
            delay(10);
        }
...
void VS1053::setTone(uint16_t value) // Set bass/treble (4 nibbles)
{
...
void VS1053::setBass(int8_t level)
{
    this->bass = level;
    setTone((treble << 12) + (treble_freq << 8) + (bass << 4) + bass_freq);
}

void VS1053::setTreble(int8_t level)
{
    this->treble = level;
    setTone((treble << 12) + (treble_freq << 8) + (bass << 4) + bass_freq);
}
...
void VS1053::stopSong() {
    ...
    for (i = 0; i < 200; i++) {
        ...
        if ((modereg & _BV(SM_CANCEL)) == 0) {
            sdi_send_fillers(2052);
            Serial.printf("Song stopped correctly after %d msec", i * 10);
            return;
        }
        delay(10);
    }
...
void VS1053::printDetails(const char *header) {
    ...
    Serial.printf(headerCr.c_str());

    Serial.printf("REG   Contents\n" );
    Serial.printf("---   -----\n" );
